...
class A2C_zeus():
    ...
    def recommend(self, ranked_list=[]):
        ...
        for i, now_rp_vec in enumerate(rp_vecs):
            ...
            for j in range(4-i):
                ...
                state.append(
                    np.sqrt(np.power(now_rp_vec-rp_vecs[i+j+1], 2).sum()))
    ...
    def learn(self, input_data=[]):
        ...
        for one_data in input_data:
            ...
            V, mu, log_var = self.model.predict(
                np.array(one_data[0]).reshape(1, -1))
        ...
        for i in range(len(s)-self.n_step):
            ...
            q += np.power(self.g, self.n_step) * \
                V_s[i+self.n_step]  # 割引した状態価値を足す

            Q.append(q)

        self.model.fit(s, epsilon, Q, self.sup_vec, self.critic_loss_coef)
...
class model():
    ...
    def fit(self, s, epsilon, Q, sup_vec, critic_loss_coef):
        ...
        inverce_sup_vec = 1/tensor(sup_vec, dtype=float)

        output_V, output_mu, output_log_var = self.net(s.float())

        actions = epsilon

        log_prob = (-(output_log_var+(actions-output_mu).pow(2) /
                      output_log_var.exp())/2).sum(axis=1)
        # ガウス方策より
